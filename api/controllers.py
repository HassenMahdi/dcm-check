...
from api.utils.storage import get_mapped_df, get_imported_data_df, save_mapped_df, save_check_results_df, \
...
def read_map_infos(params):
    """Reads longitude and latitude columns"""

    mapped_df = get_mapped_df(params["filename"], params["worksheet"], usecols=["longitude", "latitude"])
    coordinates = mapped_df.reset_index().to_json(orient='records')
...
def create_chart(params):
    """Gets chart argument and image and creates charts spec"""

    modifier_document = ModifierDocument()
    mapped_df = get_mapped_df(params["filename"], params["worksheet"], usecols=lambda col: col in {params["argument"],
                                                                                                   params["image"]})
    modifications = {"is_all": True}
    mapped_df = modifier_document.load_check_modifications(mapped_df, params["worksheet_id"], modifications)

    if params["image"]:
        mapped_df = mapped_df.apply(pd.to_numeric, args=["ignore"])
        grouped_df = mapped_df.groupby(params["argument"])
        if mapped_df[params["image"]].dtype.kind in 'biufc':
            grouped_df = grouped_df.sum().reset_index()
        else:
            grouped_df = grouped_df.count().reset_index()
    else:
        grouped_df = mapped_df[params["argument"]].value_counts().rename_axis(params["argument"]).reset_index(name='')

    chart = {"arguments": grouped_df[params["argument"]].tolist(),
             "values": grouped_df[grouped_df.columns[1]].apply(to_float).tolist(),
             "total": grouped_df[grouped_df.columns[1]].apply(to_float).sum()}

    if params["values"]:
        chart["values"][:] = map(lambda value: 100 * value/chart["total"], chart["values"])
