...
def get_check_results_df(filename, worksheet):
    ...
    results = get_dataframe_from_csv(path,delimiter=";")
    if '$_LINE_INDEX_$' in results.columns:
        results = results.drop(columns=['$_LINE_INDEX_$'])
    return results
...
def save_check_results_df(df, filename, worksheet):
    ...
    path = get_results_path(filename, worksheet, as_folder=False, create=True)
    results = df.copy()
    results['$_LINE_INDEX_$'] = df.index
    results.to_csv(path,index=False,sep=';')
